$(document).ready(function() {
    //$(document).scrollTop(function () { $("header").css("position", "unset"); });
    //$(document).scrollBottom(function () { $("header").css("position", "fixed"); });
...
function is_mobile() {
    if (($(window).width() < 968) || (navigator.userAgent.match(/(iPhone|iPod|iPad)/))) {
        return true;
    } else {
        return false;
    }
...
function checkHtmlTag(element) {
    var reg = /<(.|\n)*?>/g;
    if (reg.test(element.val()) == true) {
        element.val('');
        var ErrorText = 'Html/scriptTags do not allow ';
        alert(ErrorText);
    }
}
